...
library.createButton = function(option, parent)
	...
	option.title = library:Create("TextLabel", {
		...
		BorderColor3 = Color3.new(),
		Text = option.text .. (option.warning and " \u{26A0}\u{FE0F}" or ""), -- ⚠️
		TextColor3 = option.warning and Color3.fromRGB(255, 160, 60) or Color3.fromRGB(210, 210, 210),
		TextSize = 15,
		Font = Enum.Font.Code,
		Parent = option.main
	})
...
library.createSlider = function(option, parent)
	...
	function option:SetValue(value, nocallback)
		...
		self.value = value
		if value % 1 == 0 then
			option.title.Text = (option.text == "nil" and "" or option.text .. ": ") .. tostring(value) .. option.suffix
		else
			option.title.Text = (option.text == "nil" and "" or option.text .. ": ") .. string.format("%.1f", value) .. option.suffix
		end
		if not nocallback then
			self.callback(value)
		end
...
function library:AddTab(title, pos)
	...
	function tab:AddColumn()
		...
		function column:AddSection(title, warning)
			if warning then
				title = title .. " \u{26A0}\u{FE0F}"
			end
			...
			function UpdateAccentColor(color)
				...
				library.TitleInstance.TextColor3 = color
			end
			...
			function section:AddColor(option)
				...
				option.callback = typeof(option.callback) == "function" and option.callback or (option.flag == "Menu Accent Color" and UpdateAccentColor or function() end)
				option.open = false
			...
			function section:Init()
				...
				self.titleText = library:Create("TextLabel", {
					...
					TextSize = 15,
					Font = Enum.Font.Code,
					TextColor3 = warning and Color3.fromRGB(255, 160, 60) or Color3.new(1, 1, 1),
					Parent = self.main
				})
...
function library:Init(size)
	...
	table.insert(library.theme, library.TitleInstance)
